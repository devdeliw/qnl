...
def fluxonium(design, pos_x=0.0, pos_y=0.0, flip=False, name='left_lower'):
    ...
    if not flip: 
        pos_x = flux.node('l_pocket_bottom')[0]
        pos_y = flux.node('bottom')[1] 
        orientation = '90' 
        otg_name = 'lower_connector_otg'
    else: 
        pos_x = flux.node('l_pocket_top')[0] 
    ...
    nodes.qubit_claw = flux.node('right')
    nodes.flux_line  = flux.node('top') 
    return nodes 

def left_capactive_coupled(design, pos_x, pos_y): 
    upper_nodes = fluxonium(design=design, pos_x=pos_x, pos_y=pos_y+0.1625, flip=False, name='left_upper') 
    lower_nodes = fluxonium(design=design, pos_x=pos_x, pos_y=pos_y-0.1625, flip=True, name='left_lower') 
